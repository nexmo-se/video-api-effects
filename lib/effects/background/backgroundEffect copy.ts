...
export abstract class BackgroundEffect extends Effect {
  ...
  /* private static async _loadModel(config: ModelConfig): Promise<void> {
   ...
   */
  async processFrame(inputVideoMediaStream: MediaStreamTrack, outputFrameBuffer: HTMLCanvasElement): Promise<void> {
    /* if (!BackgroundEffect._model || !this._tflite) {
      return;
    }
    if (!inputVideoMediaStream || !outputFrameBuffer) {
      throw new Error('Missing input or output frame buffer');
    }

    const inputFrame = inputVideoMediaStream;
